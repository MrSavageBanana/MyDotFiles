...
function M:setup(opts)
	...
	-- Use explicit tag_order from config if provided, otherwise extract from hints_table
	local tag_order = {}
	if type(opts) == "table" and opts.tag_order then
		tag_order = opts.tag_order
	else
		for key, _ in pairs(st[STATE_KEY.hints_table]) do
			if key ~= "default" and key ~= "reversed" then
				table.insert(tag_order, key)
			end
		end
	end
	st[STATE_KEY.tag_keys_order] = tag_order
...
function M:redraw()
		...
		local tag_order = get_state(STATE_KEY.tag_keys_order) or {}
		
		-- Use configured tag order instead of alphabetical
...
local function show_cands_input_tags(title, input_mode, default_input_value)
	...
	if not input_mode then
		-- Build ordered candidates from configured tags
		local tag_order = get_state(STATE_KEY.tag_keys_order) or {}
		local ordered_cands = {}
		for _, tag_key in ipairs(tag_order) do
			table.insert(ordered_cands, { on = tag_key })
		end
		-- Fallback to default if no tags configured
		if #ordered_cands == 0 then
			ordered_cands = CAND_TAG_KEYS
		end
		
		toggle_tags_hints()
		choice = ya.which({ cands = ordered_cands, silent = true })
		toggle_tags_hints()
		if not choice then
			return
		end
		return ordered_cands[choice].on
	else
		toggle_tags_hints()
