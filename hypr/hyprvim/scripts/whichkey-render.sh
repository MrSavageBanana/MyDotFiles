...
open_window() {
  local _ok=0
  if [[ -n "${screen:-}" ]]; then
    # eww --screen needs a monitor index (0,1,2…), not a name like "eDP-1"
    local _idx
    _idx=$(hyprctl -j monitors 2>/dev/null \
      | jq -r --arg name "$screen" '.[] | select(.name == $name) | .id' 2>/dev/null \
      || echo "")
    if [[ -n "$_idx" ]]; then
      eww -c "$EWW_DIR" open --screen "$_idx" "$WINDOW" >/dev/null 2>&1 && _ok=1
    fi
  fi
  if [[ "$_ok" -eq 0 ]]; then
    eww -c "$EWW_DIR" open --screen 0 "$WINDOW" >/dev/null 2>&1 && _ok=1
  fi
  return $((_ok == 0 ? 1 : 0))
}
################################################################################
