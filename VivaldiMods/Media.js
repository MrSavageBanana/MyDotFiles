    ...
    if (!chrome.extension.inIncognitoContext) {
      ...
      vivaldi.prefs.onChanged.addListener(({ path, value }) => {
        ...
        buttonClose.style.display =
          (showCloseButton && asOverlayEnabled && autoClose) || !showCloseButton
            ? "none"
            : "flex";
      });
  ...
  const style = !chrome.extension.inIncognitoContext
    ? [
        ...
        'button[data-name="' +
          ...
          "); }",
        '.color-behind-tabs-off .toolbar-mainbar button[data-name="' +
          webPanelId +
          '"]:before { background-color: var(--colorAccentFg); }',
        '.button-toolbar:active button[data-name="' +
          webPanelId +
          '"]:before { transform: scale(0.9); }',
      ]
    : [
        '.button-toolbar:has(button[data-name="' +
          webPanelId +
          '"]) { display:none !important; }',
        '.draggable-button:has(button[data-name="' +
          webPanelId +
          '"]) { display:none !important; }',
      ];
